# backend/app/models/sales/sales_accounting_integration.py
...
class SaleJournalEntry(BaseModel):
    """Link Sales to Journal Entries"""
    __tablename__ = "sale_journal_entry"
    
    # Sales Reference
    sale_invoice_id = Column(Integer, ForeignKey('sale_invoice.id'), nullable=True)
    ...
    # Integration Details
    entry_type = Column(String(50), nullable=False)  # invoice, challan, return, payment, receipt
    ...
    # Accounting Details
    debit_account_id = Column(Integer, ForeignKey('chart_of_account.id'), nullable=True)  # Customer account
    credit_account_id = Column(Integer, ForeignKey('chart_of_account.id'), nullable=True)  # Sales account
    ...
    def __repr__(self):
        return f"<SaleJournalEntry(sale_id={self.sale_invoice_id}, journal_id={self.journal_entry_id})>"
...
class SalePayment(BaseModel):
    ...
    def __repr__(self):
        return f"<SalePayment(sale_id={self.sale_invoice_id}, amount={self.payment_amount})>"

class SaleAnalytic(BaseModel):
    """Link Sales to Analytic Accounting"""
    __tablename__ = "sale_analytic"
    ...
    def __repr__(self):
        return f"<SaleAnalytic(sale_id={self.sale_invoice_id}, analytic_id={self.analytic_account_id})>"

class SaleWorkflow(BaseModel):
    """Link Sales to Workflows"""
    __tablename__ = "sale_workflow"
    ...
    def __repr__(self):
        return f"<SaleWorkflow(sale_id={self.sale_invoice_id}, type='{self.workflow_type}')>"

class SaleDocument(BaseModel):
    """Link Sales to Document Management"""
    ...
    # Document Details
    document_type = Column(String(50), nullable=False)  # invoice, challan, receipt, contract, other
    ...
    def __repr__(self):
        return f"<SaleDocument(sale_id={self.sale_invoice_id}, type='{self.document_type}')>"

class SaleAuditTrail(BaseModel):
    """Link Sales to Audit Trails"""
    __tablename__ = "sale_audit_trail"
    ...
    def __repr__(self):
        return f"<SaleAuditTrail(sale_id={self.sale_invoice_id}, action='{self.action_type}')>"