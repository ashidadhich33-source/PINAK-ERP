# backend/app/models/sales/sales_enhanced_integration.py
...
class SaleInventoryIntegration(BaseModel):
    """Link Sales to Inventory Management"""
    __tablename__ = "sale_inventory_integration"
    
    # Sales Reference
    sale_invoice_id = Column(Integer, ForeignKey('sale_invoice.id'), nullable=False)
    sale_challan_id = Column(Integer, ForeignKey('sale_challan.id'), nullable=True)
    sale_return_id = Column(Integer, ForeignKey('sale_return.id'), nullable=True)
    ...
    stock_movement_type = Column(String(50), nullable=False)  # sale, return, adjustment, transfer
    ...
    sale_challan = relationship("SaleChallan")
    sale_return = relationship("SaleReturn")
    ...
    def __repr__(self):
        return f"<SaleInventoryIntegration(sale_id={self.sale_invoice_id}, item_id={self.item_id})>"

class SaleCustomerIntegration(BaseModel):
    """Link Sales to Customer Management"""
    __tablename__ = "sale_customer_integration"
    ...
    # Additional Information
    notes = Column(Text, nullable=True)
    model_metadata = Column(JSON, nullable=True)  # Additional customer data
    ...
    def __repr__(self):
        return f"<SaleCustomerIntegration(sale_id={self.sale_invoice_id}, customer_id={self.customer_id})>"

class SalePerformanceOptimization(BaseModel):
    """Link Sales to Performance Optimization"""
    __tablename__ = "sale_performance_optimization"
    ...
    def __repr__(self):
        return f"<SalePerformanceOptimization(sale_id={self.sale_invoice_id}, score={self.performance_score})>"

class SaleUserExperience(BaseModel):
    """Link Sales to User Experience Enhancement"""
    __tablename__ = "sale_user_experience"
    ...
    def __repr__(self):
        return f"<SaleUserExperience(sale_id={self.sale_invoice_id}, score={self.experience_score})>"

class SaleRealTimeSync(BaseModel):
    """Link Sales to Real-time Synchronization"""
    __tablename__ = "sale_real_time_sync"
    ...
    def __repr__(self):
        return f"<SaleRealTimeSync(sale_id={self.sale_invoice_id}, status='{self.sync_status}')>"

class SaleAnalyticsIntegration(BaseModel):
    """Link Sales to Analytics Integration"""
    __tablename__ = "sale_analytics_integration"
    ...
    def __repr__(self):
        return f"<SaleAnalyticsIntegration(sale_id={self.sale_invoice_id}, provider='{self.analytics_provider}')>"